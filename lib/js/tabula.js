...
var RectangularSelector = require('./rectangular-selector.js');
var ResizableSelection = require('./resizable-selection.js');
require('tablesorter');
require('bootstrap');
...
var PeopleView = require('./people-view.jsx');
...
Tabula.TabulaRouter = Backbone.Router.extend({
  routes: {
    "":                            "upload",
    "/":                           "upload",
    "pdf/:file_id":                "view",
...
Tabula.SearchResult = Backbone.Model.extend({
  ...
  initialize: function() {
    ...
    this.set('interesting_pages', this.get('interesting_pages') || null);
    this.set('date', this.get('date') || null);
    this.set('company', this.get('company') || null);
  }
});
...
Tabula.SearchResultView = Backbone.View.extend({
  ...
  className: 'uploaded-file',
  events: {
    'click .import-document': 'importDocument'
  },
  template: _.template($('#document-template').html().replace(/nestedscript/g, 'script')),
  initialize: function() {
    _.bindAll(this, 'render', 'importDocument');
  },
  render: function() {
    this.$el.append(this.template(this.model.attributes));
    return this;
...
Tabula.Library = Backbone.View.extend({
    events: {
        "submit form#upload": 'onUploadClick',
    },
    template: _.template( $('#uploader-template').html().replace(/nestedscript/g, 'script')),
    initialize: function(){
      _.bindAll(this, 'onUploadClick', 'render', 'renderFileLibrary');
    ...
    uploadPDF: function(files, url, data){
      ...
      _(files).each(_.bind(function(file, index){
        ...
        var file_upload = new Tabula.FileUpload({
          ...
          uploadTime: new Date(),
          uploadOrder: index,
          isOneOfMultiple: files.length != 1
        });
          ...
          success: _.bind(function (res) {
              ...
              _(statuses).each(_.bind(function(status){
                ...
                if(status.success){
                  ...
                  file_upload.set('document_id', status.document_id);
...
Tabula.LazyLoad = 0; // max number of pages around the cursor to show (2x Tabula.LazyLoad pages are shown)
...
Tabula.AutodetectedSelections = Tabula.Selections.extend({
  ...
  parse: function(response){
    ...
    var tables = [];
    var selections = _(response.page_areas_by_page).map(_.bind(function(page_tables, listIndex){
    ...
    $.ajax({
        ...
        success: _.bind(function(resp) {
          ...
          if (resp.length > 0) {
            ...
            // didn't get any tables back from the extraction method - probably guessed wrong?
            // tell the user to try specifying a method
            this.set('error_message', "No tables extracted. Try specifying a different extraction method from the sidebar.");
...
Tabula.DataView = Backbone.View.extend({  // one per query object.
  ...
  preview: function(e) {
    ...
    formdata.append("table", JSON.stringify(this.tables.state.tables[0]));
  ...
  save: function(e) {
    var formdata = new FormData($('form#download-form')[0]);
    formdata.append("people", JSON.stringify(this.peopleView.state.people));
...
Tabula.ControlPanelView = Backbone.View.extend({ // only one
  ...
  queryAllData : function(){
    ...
    // hmm.
    Tabula.pdf_view.query = new Tabula.Query({
      list_of_coords: list_of_all_coords,
      extraction_method: 'guess',
      document_id: Tabula.pdf_view.pdf_document.get('document_id')
    });
  ...
  _.extend({
    ...
    checkForAutodetectedTables: function(){
      this.pdf_document.autodetected_selections.fetch({
        success: _.bind(function(){
          if (this.pdf_document.autodetected_selections.finished) {
